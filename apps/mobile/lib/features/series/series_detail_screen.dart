...
class SeriesDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(series.value?.name ?? '')),
      body: AsyncView(
        value: series,
        builder: (s) {
          ...
          return DetailLayout(
            ...
            actions: [
              if (eps.isNotEmpty)
                FilledButton.icon(
                  ...
                  label: Text(resumeIndex >= 0 ? l10n.resume : l10n.play),
                ),
              OutlinedButton.icon(
            ...
            below: Column(
              ...
              children: [
                if (episodes.isLoading) const LinearProgressIndicator(),
                  ...
                  SizedBox(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, i) => ChoiceChip(
                        label: Text(l10n.season(seasons[i])),
                        selected: seasons[i] == selectedSeason,
                        onSelected: (_) => ref.read(_seasonProvider.notifier).set(seasons[i]),
                      ),
                    ),
                  ),
            ...
            children: [
              SizedBox(
                ...
                child: Stack(
                  fit: StackFit.expand,
                  children: [
                    ClipRRect(borderRadius: BorderRadius.circular(6), child: AppImage(episode.poster, icon: Icons.play_circle_outline)),
                    if (progress != null)
                      Positioned(left: 0, right: 0, bottom: 0, child: LinearProgressIndicator(value: progress.clamp(0, 1), minHeight: 3)),
                  ],
                ),
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('${l10n.episode(episode.episodeNum)} · ${episode.title}', maxLines: 1, overflow: TextOverflow.ellipsis, style: theme.textTheme.titleSmall),
