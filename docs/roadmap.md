...
- [ ] **Décodage `hardware` (mediacodec-copy + vo=gpu) trop lourd pour la Mi Box S gen 2 (MiTV-AFKR0), constaté par Sentry.** 1080p H.264 25 i/s en direct : ~520 images perdues en 31 s et « A/V desynchronisation » de mpv (FLUTTER-5) ; 4K HEVC HDR10 : ANR, thread principal bloqué dans `ImageReader` en attente du GPU (FLUTTER-6). Le chemin `direct` (mediacodec_embed, sans copie) avait été écarté pour des artefacts verts en 4K : à comparer sur la même box (Réglages → Décodeur vidéo → Direct) avant de changer le défaut, éventuellement par résolution.
