...
- [ ] **Compte Sentry à provisionner** : `pubspec.yaml` référence l'org `riftech` / projet `flutter`, mais `SENTRY_DSN` (app) et `SENTRY_AUTH_TOKEN` (build, upload des symboles) restent à fournir par le propriétaire — aucun secret n'est commité.
