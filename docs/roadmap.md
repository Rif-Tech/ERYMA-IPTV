...
- [ ] **Vidéo matérielle cassée sur Mi Box S gen 2 (MiTV-AFKR0, Amlogic), cause identifiée via Sentry, correctif à valider sur la box.** Test terrain : Direct = écran noir partout, Copie = saccades en FHD (~520 images perdues en 31 s, FLUTTER-5), écran vert puis ANR en 4K HDR10 (FLUTTER-6), Logiciel = OK sauf 4K. Les journaux mpv montrent `Failed to start codec OMX.amlogic.avc.decoder.awesome2 … MediaCodec start failed` en Direct (FLUTTER-D/E) et l'ANR est bloqué dans `FlutterRenderer$ImageReaderSurfaceProducer` : la surface fournie par Flutter (ImageReader, Android 10+) est incompatible avec ces décodeurs. `MainActivity` force désormais `FlutterRenderer.debugForceSurfaceProducerGlTextures` (SurfaceTexture). À revalider : Direct, puis Copie, en FHD et 4K ; ensuite choisir le décodeur par défaut (probablement Direct sur TV).
