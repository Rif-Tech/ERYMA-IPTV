...
- `SENTRY_DSN` (`--dart-define`, optionnel) : active Sentry (crashs natifs/Dart, fil d'Ariane D-pad/navigation/lecteur/DNS, audit de mise en page, Sentry Logs, IP du client). Par défaut : le projet Sentry `riftech/flutter` (le DSN est public par conception) ; `--dart-define=SENTRY_DSN=` (vide) le désactive. `SENTRY_SCREENSHOTS=true` (`--dart-define`, ou `build-release.ps1 -SentryScreenshots`) joint une capture d'écran aux événements : elle montre le contenu des playlists, à réserver aux box de test. `SENTRY_AUTH_TOKEN` (variable d'environnement locale, jamais commitée) : requis par `sentry_dart_plugin` pour publier les symboles de debug lors de `build-release.ps1` (org/projet dans `apps/mobile/pubspec.yaml`, clé `sentry:`).
